...
inline flow_vio::MatrixXd InvertPSDMatrix(const flow_vio::MatrixXd& m) {
    return m.template selfadjointView<Eigen::Upper>().llt().solve(flow_vio::MatrixXd::Identity(m.rows(), m.rows()));
}
